...
```mermaid
graph TD
    A["👤 User: Uploads Audio Files via Telegram"] --> B["🤖 Telegram Bot: Receives Files & User Info"];
    B --> C["🌐 Flask API: POST /api/v1/audio/upload (User ID, Files, Report ID)"];
    C --> D["⚙️ Backend Logic (API): Validates Request, Saves Files Temporarily, Creates Main Report Job"];
    D --> E["🔄 Backend Logic (API): Dispatches Individual Audio Processing Tasks (e.g., to Celery Queue)"];

    subgraph Asynchronous_Processing [Asynchronous Processing]
        direction LR
        F["👷 Task Worker (e.g., Celery): Picks up a single audio task"] --> G["Speech-to-Text (STT): Audio ➔ Text"];
        G -- Transcribed Text --> H["Large Language Model (LLM): Text ➔ Structured Data (Nama, Alamat, Jumlah Minuman)"];
        H -- Extracted Data / Error --> I["Task Worker Reports Result (Success or Failure for this file)"];
    end
    E -.-> F;

    I --> J["🌐 Flask API: POST /api/v1/internal/task_complete (Task ID, Report ID, Status, Data/Error)"];
    J --> K["⚙️ Backend Logic (API): Updates Individual Task Status, Aggregates Results for the Report ID"];

    K -- All Tasks for Report ID Processed --> L["⚙️ Backend Logic (API): Checks Overall Report Status"];
    L -- All/Sufficient Tasks Successful --> M["Generates Final CSV Report: Saves CSV to Storage"];
    M --> N["⚙️ Backend Logic (API): Updates Report Status: COMPLETED, Notifies Telegram Bot (or Bot Polls Status)"];

    L -- Some/All Tasks Failed --> O["Updates Report Status: FAILED/PARTIALLY_COMPLETED (May still generate partial CSV if applicable)"];
    O --> N;

    N --> P["🤖 Telegram Bot: Receives Report Status & URL (if successful)"];
    P --> Q["👤 User: Receives CSV Report/Download Link or Failure Notification via Telegram"];
